...
-- start query 10 in stream 0 using template query10.tpl -- Dist Fix: replaced subquery joins with with caluses and IN check 
...
-- start query 16 in stream 0 using template query16.tpl -- fix: added interval to days -- Dist Fix: Cte
With CS2 AS
(
...
-- start query 18 in stream 0 using template query18.tpl - Dist Fix: Used Union ALL inplace of roll up functions
SELECT i_item_id,
       ca_country,
       ca_state, 
       ca_county,
...
-- start query 27 in stream 0 using template query27.tpl -- Dist Fix: Rollup function preserved, CTE used
WITH sales_data AS (
...
-- start query 35 in stream 0 using template query35.tpl -- Dist Fix: Cte
Wwith store_sales_tmp as (
    select ss_customer_sk
    from store_sales ss
...
-- start query 36 in stream 0 using template query36.tpl -- fix: added subquery syntax (at the beginning "select * from ( " and before the last order by " ) as sub" ) -- Dist Fix: Rollup function substitued with union all and changes made accordingly
WITH base_data AS (
  SELECT 
    ss_net_profit, 
    ss_ext_sales_price,
...
-- start query 69 in stream 0 using template query69.tpl -- Dist Fix: cte
WITH store_sales_tmp AS(
  SELECT ss_customer_sk
  FROM store_sales ss
  JOIN date_dim d ON ss.ss_sold_date_sk = d.d_date_sk
...
-- start query 70 in stream 0 using template query70.tpl -- fix: subquery syntax -- Dist Fix: Rollup function substitued with union all and changes made accordingly
WITH temp_filter AS (
	SELECT s_state
...
-- start query 86 in stream 0 using template query86.tpl -- fix: subquery syntax -- Dist Fix: Rollup function substitued with union all and changes made accordingly 
...
-- start query 92 in stream 0 using template query92.tpl -- fix: added interval to days -- Dist Fix:  added the same kind of filters and table in subquery
select  
