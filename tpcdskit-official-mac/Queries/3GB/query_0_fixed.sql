...
-- start query 2 in stream 0 using template query2.tpl -- fix: added alias x
with wscs as
 (select sold_date_sk
        ,sales_price
...
-- start query 5 in stream 0 using template query5.tpl  -- fix: added interval to days
with ssr as
 (select s_store_id,
...
select case when (select count(*) 
       ...
       case when (select count(*)
                  from store_sales
                  where ss_quantity between 21 and 40) > 126605
            then (select avg(ss_ext_list_price)
                  from store_sales
       ...
       case when (select count(*)
                  from store_sales
                  where ss_quantity between 41 and 60) > 61327
            then (select avg(ss_ext_list_price)
                  from store_sales
       ...
       case when (select count(*)
                  from store_sales
                  where ss_quantity between 61 and 80) > 80224
            then (select avg(ss_ext_list_price)
                  from store_sales
       ...
       case when (select count(*)
                  from store_sales
                  where ss_quantity between 81 and 100) > 73097
            then (select avg(ss_ext_list_price)
                  from store_sales
...
select  
 ...
 where
  ...
  ca_county in ('Williamson County','Warren County','Butler County','Cherokee County','Putnam County') and
...
-- start query 12 in stream 0 using template query12.tpl -- fix: added interval to days
select  i_item_id
      ,i_item_desc 
      ,i_category 
      ,i_class 
...
where 
	ws_item_sk = i_item_sk 
  	and i_category in ('Children', 'Music', 'Jewelry')
  	and ws_sold_date_sk = d_date_sk
	and d_date between cast('1998-04-23' as date) 
				and (cast('1998-04-23' as date) + interval '30 days')
group by 
	i_item_id
        ,i_item_desc 
        ,i_category
...
select avg(ss_quantity)
 ...
 and((ss_addr_sk = ca_address_sk
  ...
  and ca_state in ('IA', 'AL', 'SC')
  and ss_net_profit between 100 and 200  
     ) or
     (ss_addr_sk = ca_address_sk
  and ca_country = 'United States'
  and ca_state in ('TX', 'LA', 'KS')
  and ss_net_profit between 150 and 300  
     ) or
     (ss_addr_sk = ca_address_sk
  and ca_country = 'United States'
  and ca_state in ('VA', 'CO', 'NJ')
  and ss_net_profit between 50 and 250  
     ))
;

-- end query 13 in stream 0 using template query13.tpl
-- start query 14 in stream 0 using template query14.tpl --fix: added alias s
with  cross_items as
 ...
 where ws_item_sk = iws.i_item_sk
   ...
   and d3.d_year between 1998 AND 1998 + 2) s
...
-- start query 16 in stream 0 using template query16.tpl -- fix: added interval to days
select  
...
select  i_item_id,
 ...
 where cs_sold_date_sk = d_date_sk and
       ...
       c_birth_month in (8,7,2,9,1,12) and
       d_year = 2002 and
       ca_state in ('VA','VT','OH'
                   ,'MT','TX','IN','KS')
...
-- start query 20 in stream 0 using template query20.tpl -- fix: added interval to days
select  i_item_id
       ,i_item_desc 
       ,i_category 
       ,i_class 
...
-- start query 21 in stream 0 using template query21.tpl -- fix: added interval to days
select  *
 from(select w_warehouse_name
   ...
   where i_current_price between 0.99 and 1.49
     ...
     and d_date between (cast ('1998-03-07' as date) - interval '30 days')
                    and (cast ('1998-03-07' as date) + interval '30 days')
...
-- start query 30 in stream 0 using template query30.tpl -- fix: updated column name from c_last_review_date_sk to c_last_review_date
with customer_total_return as
...
-- start query 36 in stream 0 using template query36.tpl -- fix: added subquery syntax (at the beginning "select * from ( " and before the last order by " ) as sub" )
select * from (
select  
...
select  i_item_id
 ...
 and d_date between cast('2000-04-13' as date) and (cast('2000-04-13' as date) +  interval '60 days')
 and i_manufact_id in (670,756,751,871)
...
-- start query 40 in stream 0 using template query40.tpl -- fix: added interval to days
select  
   w_state
  ,i_item_id
 ...
 and cs_sold_date_sk    = d_date_sk
 and d_date between (cast ('1999-02-26' as date) - interval '30 days')
                and (cast ('1999-02-26' as date) + interval '30 days') 
 group by
    w_state,i_item_id
...
-- start query 49 in stream 0 using template query49.tpl -- fix: added alias x
...
with ss as (
 ...
 where i_item_id in (select
     i_item_id
from item
where i_color in ('black','green','firebrick'))
 and     ss_item_sk              = i_item_sk
 and     ss_sold_date_sk         = d_date_sk
 and     d_year                  = 2000
 and     d_moy                   = 3
...
cross_sales as
 (select i_product_name product_name
  ...
  WHERE  ss_store_sk = s_store_sk AND
         ...
         i_color in ('khaki','peach','navy','honeydew','cyan','floral') and
...
select   
 ...
 from (
 	...
 	and ws_ship_mode_sk = sm_ship_mode_sk
        and d_year = 1999
 	and t_time between 14944 and 14944+28800 
 	and sm_carrier in ('MSC','UPS')
     group by 
        w_warehouse_name
 	,w_warehouse_sq_ft
 	,w_city
...
-- start query 70 in stream 0 using template query70.tpl -- fix: subquery syntax
select * from (
select  
...
-- start query 77 in stream 0 using template query77.tpl -- fix: added interval to days
with ss as
 (select s_store_sk,
 ...
 where ss_sold_date_sk = d_date_sk
       and d_date between cast('1998-08-24' as date) 
                  and (cast('1998-08-24' as date) +  interval '30 days') 
       and ss_store_sk = s_store_sk
 group by s_store_sk)
 ,
 sr as
...
-- start query 80 in stream 0 using template query80.tpl -- fix: added interval to days
with ssr as
 ...
 where ss_sold_date_sk = d_date_sk
       and d_date between cast('1998-08-10' as date) 
                  and (cast('1998-08-10' as date) + interval '30 days')
       and ss_store_sk = s_store_sk
       and ss_item_sk = i_item_sk
...
select  i_item_id
 ...
 and i_manufact_id in (842,515,856,134)
 and inv_quantity_on_hand between 100 and 500
 and ss_item_sk = i_item_sk
 group by i_item_id,i_item_desc,i_current_price
 order by i_item_id
...
with sr_items as
 ...
 and   d_date    in 
	...
	where d_week_seq in 
		(select d_week_seq
		from date_dim
	  where d_date in ('1999-06-29','1999-10-23','1999-11-16')))
...
select  c_customer_id as customer_id
 ...
 where ca_city	        =  'Stringtown'
   and c_current_addr_sk = ca_address_sk
   and ib_lower_bound   >=  53421
   and ib_upper_bound   <=  53421 + 50000
...
-- start query 86 in stream 0 using template query86.tpl -- fix: subquery syntax 
select * from ( 
select   
...
where ss_item_sk = i_item_sk and
      ...
      ss_store_sk = s_store_sk and
      d_year in (1999) and
        ((i_category in ('Music','Home','Books') and
          i_class in ('classical','blinds/shades','home repair')
         )
      or (i_category in ('Children','Women','Men') and
          i_class in ('infants','swimwear','sports-apparel') 
        ))
...
-- start query 92 in stream 0 using template query92.tpl -- fix: added interval to days
select  
...
and d_date between '2002-03-20' and 
        (cast('2002-03-20' as date) + interval '90 days')
...
-- start query 94 in stream 0 using template query94.tpl -- fix: added interval to days
select  
...
-- start query 98 in stream 0 using template query98.tpl -- fix: added interval to days
select i_item_id
      ,i_item_desc 
      ,i_category 
      ,i_class 
